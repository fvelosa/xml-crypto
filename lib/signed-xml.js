...
SignedXml.prototype.validateSignatureValue = function(doc) {
  ...
  // Some IdP sign like ForgeRock need to manipulate the digest with the \r \n chars to pass the signature validation
  const digestRegex = /<([^:]*:)?DigestValue>(.|\r|\n|\t)+<\/([^:]*:)?DigestValue>/gi;
  const digest = signedInfoCanon.match(digestRegex)[0]

  const digestPosix = digest.replace(/\n/gi, '&#xD;\n')
  const digestWindows = digest.replace(/\r/gi, '&#xD;\r')

  const signedInfoCanonPosix = signedInfoCanon.replace(digestRegex, digestPosix);
  const resPosix = signer.verifySignature(signedInfoCanonPosix, this.signingKey, this.signatureValue)

  const signedInfoCanonWindows = signedInfoCanon.replace(digestRegex, digestWindows);
  const resWindows = signer.verifySignature(signedInfoCanonWindows, this.signingKey, this.signatureValue)

  if (!res && !resPosix && !resWindows) this.validationErrors.push("invalid signature: the signature value " +
                                        this.signatureValue + " is incorrect")
  return res || resPosix || resWindows
}
